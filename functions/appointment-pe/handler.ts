...
/**
 * Lambda handler for processing appointments in Peru (PE)
...
export const main: SQSHandler = async (event: SQSEvent): Promise<void> => {
  ...
  logger.info(LOG_EVENTS.PE_APPOINTMENT_PROCESSING_STARTED.message, {
    logId: LOG_EVENTS.PE_APPOINTMENT_PROCESSING_STARTED.logId,
    requestId,
    recordCount: event.Records.length,
    eventSource: 'SQS',
    country: COUNTRY_PROCESSING.PERU
  });

  for (const record of event.Records) {
    try {
      logger.info('Processing SQS record for PE', {
      ...
      const processDto: ProcessAppointmentDto = {
        appointmentId,
        countryISO: COUNTRY_PROCESSING.PERU as 'PE',
        insuredId,
        scheduleId: Number(scheduleId)
      };

      logger.info('Processing appointment for PE', {
      ...
      logger.info(LOG_EVENTS.PE_APPOINTMENT_PROCESSED.message, {
      ...
      logger.error(LOG_EVENTS.PE_APPOINTMENT_PROCESSING_ERROR.message, {
        logId: LOG_EVENTS.PE_APPOINTMENT_PROCESSING_ERROR.logId,
        messageId: record.messageId,
  ...
  logger.info('PE appointment processing completed', {
    logId: 'pe-processing-completed',
